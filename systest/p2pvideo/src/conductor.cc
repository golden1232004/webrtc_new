...
//  main_wnd_->MessageBox("Error", "Failed to initialize PeerConnectionFactory", true);
	cout << "Failed to initialize PeerConnectionFactory" << endl;
    DeletePeerConnection();
    return false;
  }
...
//    main_wnd_->MessageBox("Error", "CreatePeerConnection failed", true);
    cout << "CreatePeerConnection failed" << endl;
	DeletePeerConnection();
  }
  AddStreams();
  return peer_connection_.get() != NULL;
...
bool Conductor::CreatePeerConnection(bool dtls) {
  ...
  server.username = "1466242573:u1";
  server.password = "SOaW2K7VrgAM8UfXsbFfOmjCEHQ=";
...
void Conductor::OnMessageFromPeer(int peer_id, const std::string& message) {
  std::cout<<"OnMessageFromPeer:"<<peer_id<<endl; 
...
void Conductor::ConnectToPeer(int peer_id) {
	std::cout<<"ConnectTOPeer:"<<peer_id<<endl;
  ASSERT(peer_id_ == -1);
  ASSERT(peer_id != -1);

  if (peer_connection_.get()) {
//    main_wnd_->MessageBox("Error", "We only support connecting to one peer at a time", true);
      cout << "only support connecting to one peer at a time" << endl;
	  return;
  }
...
//    main_wnd_->MessageBox("Error", "Failed to initialize PeerConnection", true);
	  cout << "Failed to initialize PeerConnection" << endl;
  }
}
